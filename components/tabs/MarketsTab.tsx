...
import { Globe, Plus, Search, Edit2, Trash2, Activity, X, ShieldCheck, ChevronDown, MapPin, TrendingUp } from 'lucide-react';
...
const STATUS_META = {
  Active: { color: '#10b981', bg: 'bg-emerald-50', text: 'text-emerald-600', label: 'Active' },
  Inactive: { color: '#94a3b8', bg: 'bg-slate-100', text: 'text-slate-400', label: 'Inactive' },
};
...
const MarketsTab: React.FC<MarketsTabProps> = ({ availableBrands, selectedBrandId, setSelectedBrandId, currentUser }) => {
  ...
  const fetchMarkets = async () => {
    try {
      ...
      const marketsData = querySnapshot.docs.map(d => ({ id: d.id, ...d.data() })) as Market[];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormData({ name: '', region: '', status: 'Active', competitors: '', market_trends: '', brand_id: '' });
      fetchMarkets();
    } catch (error) {
  ...
  }), [markets, availableBrands, selectedBrandId, searchTerm]);

  const stats = useMemo(() => ({
    total: filteredMarkets.length,
    active: filteredMarkets.filter(m => m.status === 'Active').length,
    inactive: filteredMarkets.filter(m => m.status === 'Inactive').length,
    regions: new Set(filteredMarkets.map(m => m.region).filter(Boolean)).size,
  }), [filteredMarkets]);

  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <SectionHeader title={t('markets.title')} subtitle={t('markets.subtitle')}>
        <button
          onClick={() => {
            setFormData({ name: '', region: '', status: 'Active', competitors: '', market_trends: '', brand_id: selectedBrandId });
            setIsModalOpen(true);
          }}
      ...
      <div className="flex flex-wrap gap-3">
        <div className="flex items-center gap-2.5 px-5 py-3 bg-white rounded-2xl border border-slate-100 shadow-sm">
          ...
          <span className="text-label-caps">{stats.total} THỊ TRƯỜNG</span>
        </div>
        <div className="flex items-center gap-2.5 px-5 py-3 bg-emerald-50 rounded-2xl border border-emerald-100">
          <div className="w-2 h-2 rounded-full bg-emerald-500" />
          <span className="text-label-caps !text-emerald-600">{stats.active} ĐANG HOẠT ĐỘNG</span>
        </div>
        <div className="flex items-center gap-2.5 px-5 py-3 bg-slate-50 rounded-2xl border border-slate-100">
          <div className="w-2 h-2 rounded-full bg-slate-400" />
          <span className="text-label-caps">{stats.inactive} KHÔNG HOẠT ĐỘNG</span>
        </div>
        <div className="flex items-center gap-2.5 px-5 py-3 bg-amber-50 rounded-2xl border border-amber-100">
          <MapPin className="w-3 h-3 text-amber-500" />
          <span className="text-label-caps !text-amber-600">{stats.regions} KHU VỰC</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {filteredMarkets.map((market, idx) => {
          const meta = STATUS_META[market.status] || STATUS_META.Active;
          return (
            <div
              key={market.id}
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <div className="w-11 h-11 rounded-xl flex items-center justify-center shrink-0" style={{ background: meta.color + '18' }}>
                      <Globe className="w-5 h-5" style={{ color: meta.color }} />
                    </div>
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                        {market.region && (
                          <span className="flex items-center gap-1 px-2.5 py-0.5 bg-amber-50 rounded-full text-[9px] font-black uppercase tracking-widest text-amber-600">
                  ...
                  <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                    <button
                      onClick={() => {
                        ...
                        setFormData({ name: market.name, region: market.region, status: market.status, competitors: market.competitors, market_trends: market.market_trends, brand_id: market.brand_id });
                        setIsModalOpen(true);
                      }}
                ...
                <div className="grid grid-cols-2 gap-4 pt-4 border-t border-slate-50">
                  <div>
                    <div className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-1 flex items-center gap-1">
                      ...
                      {t('markets.competitors')}
                    </div>
                    <p className="text-xs text-slate-500 leading-relaxed line-clamp-2">{market.competitors}</p>
                  </div>
                  <div>
                    <div className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-1 flex items-center gap-1">
                    ...
                    <p className="text-xs text-slate-500 leading-relaxed line-clamp-2">{market.market_trends}</p>
                  </div>
                </div>
              </div>
            </div>
      ...
      {filteredMarkets.length === 0 && !loading && (
        <div className="flex flex-col items-center justify-center py-32 bg-white rounded-2xl border border-dashed border-slate-200 shadow-sm">
          ...
          <p className="text-label-caps opacity-40">{t('markets.empty_text')}</p>
        </div>
      )}
              ...
              <div className="flex items-center gap-6">
                ...
                <div>
                  <div className="inline-block px-4 py-1.5 bg-slate-100/80 rounded-full mb-2">
                    <div className="text-label-caps">{t('markets.analysis')}</div>
                  </div>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden">
              <div className="p-10 pt-0 space-y-8 overflow-y-auto custom-scrollbar flex-1">
                <div className="grid grid-cols-2 gap-8">
                  <div className="col-span-2">
                    <label className="block text-label-caps mb-3 ml-1">{t('markets.modal.name_label')}</label>
                    <input required className="w-full px-6 py-5 bg-white border border-slate-100 rounded-[2rem] focus:border-blue-500 focus:ring-4 focus:ring-blue-500/5 focus:outline-none transition-all text-navy font-bold text-lg placeholder:text-slate-300 shadow-sm" placeholder={t('markets.modal.name_placeholder')} value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} />
                  </div>
                  <div>
                    <label className="block text-label-caps mb-3 ml-1">{t('markets.modal.region_label')}</label>
                    <input required className="w-full px-6 py-5 bg-white border border-slate-100 rounded-[2rem] focus:border-blue-500 focus:ring-4 focus:ring-blue-500/5 focus:outline-none transition-all text-navy font-bold text-lg placeholder:text-slate-300 shadow-sm" placeholder={t('markets.modal.region_placeholder')} value={formData.region} onChange={e => setFormData({ ...formData, region: e.target.value })} />
                  </div>
                  <div>
                    <label className="block text-label-caps mb-3 ml-1">{t('markets.modal.status_label')}</label>
                    <div className="relative">
                      <select className="w-full px-6 py-5 bg-white border border-slate-100 rounded-[2rem] focus:border-blue-500 focus:ring-4 focus:ring-blue-500/5 focus:outline-none transition-all text-navy font-bold text-lg appearance-none shadow-sm" value={formData.status} onChange={e => setFormData({ ...formData, status: e.target.value as any })}>
                  ...
                  <div className="col-span-2">
                    <label className="block text-label-caps mb-3 ml-1">{t('markets.modal.competitors_label')}</label>
                    <textarea className="w-full px-6 py-5 bg-white border border-slate-100 rounded-[2rem] focus:border-blue-500 focus:ring-4 focus:ring-blue-500/5 focus:outline-none transition-all text-navy font-bold text-lg placeholder:text-slate-300 h-32 resize-none shadow-sm" placeholder={t('markets.modal.competitors_placeholder')} value={formData.competitors} onChange={e => setFormData({ ...formData, competitors: e.target.value })} />
                  </div>
                  <div className="col-span-2">
                    <label className="block text-label-caps mb-3 ml-1">{t('markets.modal.trends_label')}</label>
                    <textarea className="w-full px-6 py-5 bg-white border border-slate-100 rounded-[2rem] focus:border-blue-500 focus:ring-4 focus:ring-blue-500/5 focus:outline-none transition-all text-navy font-bold text-lg placeholder:text-slate-300 h-32 resize-none shadow-sm" placeholder={t('markets.modal.trends_placeholder')} value={formData.market_trends} onChange={e => setFormData({ ...formData, market_trends: e.target.value })} />
                  </div>
                </div>
              </div>
              <div className="flex items-center justify-end gap-10 p-6 px-10 border-t border-slate-100 shrink-0 bg-white shadow-[0_-10px_30px_rgba(0,0,0,0.02)]">
                ...
                <button type="submit" className="px-10 py-5 bg-navy hover:bg-navy/90 text-white rounded-[1.5rem] transition-all shadow-xl shadow-navy/20 flex items-center gap-3 group text-label-caps !text-white">
                  ...
                  <span>{editingMarket ? t('common.save_changes') : t('common.add_new')}</span>
                </button>
              </div>
            </form>
          </div>
